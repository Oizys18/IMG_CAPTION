...
- 컴퓨터의 환경에 따라 config 를 통해 batch, epoch 사이즈를 조정할 수 있습니다. 
  ...
  📢 기본으로 설정된 값은 **sampling 실행(sampling size=0.01) ,  epoch=6** 입니다.
...
- 정규화한 이미지를 보여줍니다.
  ...
  ```bash
  $python doc/img_normalization_test.py
  ```
...
- 텍스트 데이터 전처리
  ...
  ```bash
  $python doc/text_tokenizer_test.py
  ```
...
- `demo.py` 실행 시 아래와 같은 결과물을 확인할 수 있습니다.
  ...
  📢 predict.py 가 먼저 실행이 되어, 임의의 이미지에 캡션이 추가된 이미지가 먼저 출력됩니다. 이후에 아래와 같은 사진 선택 화면이 뜨고 있습니다.
